...
class Trajectory:
    ...
    def init_parameters(self):
        ...
        self.Q = np.mat(np.eye(self.track_dim))*100
        self.P = np.mat(np.eye(self.track_dim-6))*0.001
    ...
    def state_prediction(self,timestamp):
        ...
        if timestamp-1 in self.trajectory.keys():
            if self.trajectory[timestamp-1].updated_state is not None:
                current_prediction_score = previous_prediction_score * (1 - 0.03*15)
            else:
                current_prediction_score = previous_prediction_score * (1 - 0.03)
        else:
            current_prediction_score = previous_prediction_score * (1 - 0.03)
    ...
    def state_update(self,
        ...
        if self.consecutive_missed_num>1:
            ...
            current_ob.prediction_score = current_ob.prediction_score + 0.03*10*(self.sigmoid(score))
        else:
            current_ob.prediction_score = current_ob.prediction_score + 0.03*(self.sigmoid(score))
